  ...
  return (
    ...
    <aside id="sidebar" className=" md:w-[20%] h-screen py-4 text-white">
      ...
      <div className="pt-4 flex h-full">
        <ScrollArea className="h-[35rem] w-full overflow-visible z-20" type="scroll">
        ...
        <nav className="flex flex-col h-full justify-center gap-3 w-[20rem] p-3 z-20">
          {sideBarItems.map((item) => (
            ...
            className={({ isActive }) =>
            cn(` flex gap-3 items-center p-2 w-11/12 z-20 hover:bg-gray-600`, {
              ...
              {({ isActive }) => (
                <>
                  ...
                  <p className={cn("text-base", {"font-semibold": isActive})}>{item.title}</p>
                </>
              )}
            </NavLink>
          ))}
        ...
        <div className="relative h-0 z-0">
          <img src={group} className="w-[20rem] h-[20rem] opacity-70 fixed bottom-0 z-0 -left-10" />
      </div>
    </aside>
  );
}
