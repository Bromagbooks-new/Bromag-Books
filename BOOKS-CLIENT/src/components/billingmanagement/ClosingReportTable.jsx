...
const ClosingReportTable = ({ reports }) => {
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-[#1F303C] hover:bg-[#1F303C]">
              ...
              <TableHead className="text-white">Total Orders</TableHead>
              <TableHead className="text-center text-white ">Total Amount</TableHead>
              <TableHead className="text-center text-white ">Online Total</TableHead>
              <TableHead className="text-center text-white ">Takeaway Total</TableHead>
              <TableHead className="text-center text-white ">Dinein Total</TableHead>
              <TableHead className="text-center text-white  rounded-tr-lg">Cash</TableHead>
            </TableRow>
          </TableHeader>
          <TableBody className="bg-[#F4FAFF]">
            {filteredReports.map((report, index) => (
              <TableRow key={report._id}>
                ...
                <TableCell className="py-3 text-center">{new Date(report.createdAt).toLocaleDateString()}</TableCell>
                <TableCell className="py-3 text-center">{report.totalBills}</TableCell>
                <TableCell className="py-3 text-center">₹{report.totalAmount}</TableCell>
                <TableCell className="py-3 text-center">₹{report.totalOnlineAmount}</TableCell>
                <TableCell className="py-3 text-center">₹{report.totalTakeawayAmount}</TableCell>
                <TableCell className="py-3 text-center">₹{report.totalDineinAmount}</TableCell>
                <TableCell className="text-center py-3">
                  <Dialog>
                    <DialogTrigger>
                      <Button className="bg-secondary h-8">View</Button>
                    </DialogTrigger>
                    <OpeningCashDenominationDialog denomination={report.cashDenomination} date={new Date(report.createdAt).toLocaleDateString()} />
                  </Dialog>
                </TableCell>
              </TableRow>
            ))}
...
export default ClosingReportTable;
