...
import { UpdateTableForTableManagement } from "@/config/routeApi/owner";
import { toastError, toastSuccess } from "@/helpers/helpers";

const aggregatorSchema = z.object({
    tableNumber: z.number().min(1, { message: "Kindly enter the table number" }),
    numberOfSeats: z.string().min(1, { message: "Kindly select the table seating type" }),
});

const UpdateTableForm = ({ singleTableData }) => {
    // console.log('singleTableData:', singleTableData)
    const form = useForm({
        resolver: zodResolver(aggregatorSchema),
        defaultValues: {
            tableNumber: singleTableData?.tableNumber,
            numberOfSeats: singleTableData?.numberOfSeats,
        },
    });
    // console.log("from :",form.watch());
    const { tableId } = useParams();

    const navigate = useNavigate();
    const onhandleUpdateSubmit = async (data) => {
        console.log(data);

        try {
            const response = await UpdateTableForTableManagement(tableId, data);
    ...
    return (
        <Form {...form}>
            <form
                onSubmit={form.handleSubmit(onhandleUpdateSubmit)}
