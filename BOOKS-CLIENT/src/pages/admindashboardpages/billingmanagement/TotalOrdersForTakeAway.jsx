...
const TotalOrdersForTakeAway = () => {
    ...
    useEffect(() => {
        if (isInitialMount.current) {
            ...
            getTotalOrdersForTakeAwayFn();
        }
    }, [page])

    const getTotalOrdersForTakeAwayFn = async () => {
        if (loader) return;
        console.log("Here");
        setLoader(true);
        try {
            const { data } = await GetTotalBillsEitherForTakeAwayOrOnlineOrders(page, "takeaway");
            console.log('data:', data)
            if (data?.success) {
                setReports(data?.totalOrdersBillData);
            }
        } catch (error) {
            console.log('error in getTotalOrdersForTakeAwayFn:', error)
        } finally {
            setLoader(false);
        }
    }
...
export default TotalOrdersForTakeAway;